...
def stabilize(pid, cluster_name):
    while True:
        try:
            status = eks.describe_fargate_profile(clusterName=cluster_name, fargateProfileName=pid)['fargateProfile']['status']
...
def delete(event, _):
    ...
    try:
        ...
        return
    status = stabilize(event['PhysicalResourceId'], event['ResourceProperties']['ClusterName'])
    if status != 'DELETED':
        raise Exception(f"Fargate profile {event['PhysicalResourceId']} status is {status}")
